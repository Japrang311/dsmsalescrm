...
- Local Supabase project ID is `DSM_SALES_WEB_APP_V2`. Never run `supabase link`, `supabase db push`, or another non-local mutation without explicit approval naming the target project. RFQ-retirement migrations `20260725151142` and `20260725152241` are committed but must still be treated as remote-pending unless `supabase migration list --linked` proves otherwise in the current session.
