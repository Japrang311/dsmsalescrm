...
describe("canUseAiSummary", () => {
  test("allows exactly the one pilot account", () => {
    expect(canUseAiSummary("adhitya@dutasolusimetalindo.com")).toBe(true);
    expect(AI_SUMMARY_ALLOWED_EMAILS).toHaveLength(1);
  });
  ...
  test("rejects everyone else", () => {
    expect(canUseAiSummary("triyanto@dutasolusimetalindo.com")).toBe(false);
