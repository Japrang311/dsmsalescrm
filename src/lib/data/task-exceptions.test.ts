...
describe("manager task selectors", () => {
  ...
  test("Team Exceptions contains only active Sales-owned escalated tasks", () => {
    const tasks = [
      ...
      task("waiting-external-threshold", "sales-2", "Waiting External", "Escalated"),
    ...
    expect(filterManagerTeamExceptions(tasks, ownersById).map((t) => t.id)).toEqual([
      "threshold",
