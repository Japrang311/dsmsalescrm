-- Re-add description column to sales_order_items.
-- product_name already holds the main item name (from the merge migration).
-- description is now empty/NULL for existing rows; users fill it going forward.

ALTER TABLE sales_order_items ADD COLUMN description text;

-- Update the import RPC to write description for sales order items again
    ...
    LOOP
      ...
      ELSE
        INSERT INTO public.sales_order_items (
          sales_order_id, product_name, description,
          qty, uom, unit_price, line_total, line_position
        ) VALUES (
          v_id,
