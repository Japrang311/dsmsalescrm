-- Repair the historical import RPC after the product-name migrations replaced
-- its body with pre-normalization column and counter names. Keep the service-
-- role-only, single-transaction import boundary intact.
...
begin
  ...
  -- Resolve revision links only after every imported header exists so input
  -- order cannot create transiently invalid chains.
  for v_document in
    select value
